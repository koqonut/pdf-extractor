...
{"items": [{"name": "...", "price": "..."}]}"""
        ...
        return {"success": False, "engine": "Claude 3.5 Sonnet", "error": str(e)}
    ...
    if successful_local:
        ...
        if cloud_result.get("success"):
            ...
            if best_local_items >= cloud_items * 0.9:  # Within 90% of cloud
                ...
                print(
                    f"   Reason: Achieves {(best_local_items/cloud_items)*100:.0f}% of cloud accuracy at $0 cost"
                )
                ...
                print("   - Use Claude API for low-confidence cases")
                print("   - Estimated cost: ~30% of full cloud ($0-100 per 1000 flyers)")
            else:
                print("\n❌ RECOMMENDATION: Use Claude API (cloud)")
                print(
                    f"   Reason: Local methods only achieve {(best_local_items/cloud_items)*100:.0f}% accuracy"
                )
                print("   Cost is worth it for reliability")
